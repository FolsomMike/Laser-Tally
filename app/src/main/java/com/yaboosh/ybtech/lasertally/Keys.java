/******************************************************************************
* Title: Keys.java
* Author: Hunter Schoonover
* Create Date: 09/27/14
* Last Edit: 
*
* Purpose:
*
* This class is used to store static variables that serve as keys for different
* operations.
*
*/

//-----------------------------------------------------------------------------


package com.yaboosh.ybtech.lasertally;

//-----------------------------------------------------------------------------

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//class Keys
//

public class Keys {

    public static final int ACTIVITY_RESULT_JOB_INFO = 1230;
    public static final int ACTIVITY_RESULT_TABLE_ROW_EDITOR = 1231;
    public static final int ACTIVITY_RESULT_VERIFY_ACTION = 1233;
    public static final int ACTIVITY_RESULT_ENABLE_BT = 1234;
    public static final int ACTIVITY_RESULT_MORE = 1235;
    public static final int ACTIVITY_RESULT_MORE_OPTIONS = 1236;

    public static final String SHARED_SETTINGS_KEY = "SHARED_SETTINGS_KEY";

    //Unit system keys
    public static final String IMPERIAL_MODE = "IMPERIAL_MODE";
    public static final String METRIC_MODE = "METRIC_MODE";
    //end of unit system keys

    // Job Info Keys
    public static final String JOBS_HANDLER_KEY = "JOBS_HANDLER_KEY";

    public static final String EDIT_JOB_INFO_ACTIVITY_MODE_KEY = "EDIT_JOB_INFO_ACTIVITY_MODE_KEY";

}//end of class Keys
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
